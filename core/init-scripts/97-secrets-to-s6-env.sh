...
# (opencode, bapx-master, triggers) have their env vars.
#
...
SEED_FILE="/ephemeral/bapx-master/seed-env.json"
...
if [ -f "$BOOTSTRAP_FILE" ] && [ -d "$S6_ENV_DIR" ]; then
  ...
  bun -e "
    ...
    if (n > 0) console.log('[Bapx] Restored ' + n + ' core env var(s) from bootstrap');
  " || echo "[Bapx] WARN: bootstrap restore failed (non-fatal)"
fi

# ── 3. Seed template keys (first run only) ──────────────────────────────────
if [ ! -f "$SECRETS_FILE" ] && [ -f "$SEED_FILE" ]; then
  echo "[Bapx] First run — seeding template keys into SecretStore"
  bun -e "
    const { SecretStore } = require('/ephemeral/bapx-master/src/services/secret-store.ts');
    ...
    console.log('[Bapx] Seeded ' + n + ' template keys');
  " || echo "[Bapx] WARN: template key seed failed"
fi
...
if [ -f "$SECRETS_FILE" ]; then
  echo "[Bapx] Syncing secrets into s6 container environment..."
  bun "/ephemeral/bapx-master/src/scripts/sync-s6-env.ts" || echo "[Bapx] WARN: secret sync failed"
else
  echo "[Bapx] No secrets file yet — skipping secret sync"
fi

# ── 5. Fix s6 env dir ownership ─────────────────────────────────────────────
# Sync ran as root. bapx-master (abc) needs write access at runtime.
if [ -d "$S6_ENV_DIR" ]; then
  chown -R abc:users "$S6_ENV_DIR"
fi

echo "[Bapx] Secrets recovery complete"
