...
export const config = {
  ...
  PORT: parseInt(process.env.BAPX_MASTER_PORT || '8000'),
  ...
  // ─── Bapx Backend ─────────────────────────────────────────────────────────
  // BAPX_API_URL: base URL of bapx-api. Source of truth is the secrets-manager-
  // backed s6 env file when present; process.env/.env are fallbacks for native dev.
  get BAPX_API_URL() { return getEnv('BAPX_API_URL') || 'http://localhost:8008' },

  // BAPX_TOKEN — direction: sandbox → bapx-api.
  ...
  get BAPX_TOKEN() { return getEnv('BAPX_TOKEN') || '' },

  // Feature flag: enable or disable local deployment routes (/bapx/deploy/*)
  BAPX_DEPLOYMENTS_ENABLED: process.env.BAPX_DEPLOYMENTS_ENABLED === 'true',

  // Secret storage
  SECRET_FILE_PATH: process.env.SECRET_FILE_PATH || `${process.env.BAPX_PERSISTENT_ROOT || '/persistent'}/secrets/.secrets.json`,
  SALT_FILE_PATH: process.env.SALT_FILE_PATH || `${process.env.BAPX_PERSISTENT_ROOT || '/persistent'}/secrets/.salt`,
  ENCRYPTION_KEY_PATH: process.env.ENCRYPTION_KEY_PATH || `${process.env.BAPX_PERSISTENT_ROOT || '/persistent'}/secrets/.encryption-key`,
  ...
  get INTERNAL_SERVICE_KEY(): string {
    ...
    const tokenAlias = getEnv('BAPX_TOKEN') || process.env.BAPX_TOKEN || ''
    if (tokenAlias) {
    ...
    if (!process.env.INTERNAL_SERVICE_KEY) {
      console.warn(
        '[Bapx Master] WARNING: No INTERNAL_SERVICE_KEY or BAPX_TOKEN available.\n' +
