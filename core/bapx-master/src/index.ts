...
// Must run BEFORE SecretStore because BAPX_TOKEN is the encryption key.
loadBootstrapEnv()
...
// to ensure they're always available regardless of how the process was started.
{
  ...
  for (const key of CORE_VARS) {
    // Use injected env var, but fall back to a sane default for BAPX_API_URL
    // so OpenCode's {env:BAPX_API_URL} substitution always produces a valid URL.
    let val: string | undefined = process.env[key]
    if (!val && key === 'BAPX_API_URL') {
      val = 'http://localhost:8008'
    }
    if (!val && key === 'BAPX_YOLO_API_KEY') {
      val = process.env.BAPX_TOKEN
    }
    if (!val && key === 'BAPX_YOLO_URL') {
...
const authSyncDisabled = process.env.BAPX_DISABLE_AUTH_SYNC === 'true'
...
if (process.env.BAPX_DISABLE_CORE_SUPERVISOR !== 'true') {
...
function inspectOpencodeState() {
  ...
  const persistentRoot = process.env.BAPX_PERSISTENT_ROOT || null
...
app.get('/bapx/internal/session-scopes/:sessionId', async (c) => {
  ...
  try {
    const workspace =
      process.env.BAPX_WORKSPACE?.trim() ||
