...
/**
 * Derive the v1 API base URL from BAPX_API_URL.
 * BAPX_API_URL may contain a path suffix (e.g. "/v1/router") used by other
 * subsystems. We only need the origin (scheme + host + port) for direct API calls.
 */
function getApiV1(): string {
  try {
    return new URL(config.BAPX_API_URL).origin + '/v1'
  } catch {
    // Fallback: strip any path and append /v1
    return config.BAPX_API_URL.replace(/\/+$/, '').replace(/\/v1\/.*$/, '') + '/v1'
  }
}
...
/**
 * Guard: ensure BAPX_TOKEN is available before proxying to bapx-api.
...
pipedreamRouter.use('*', async (c, next) => {
  if (!config.BAPX_TOKEN) {
    console.error('[Pipedream] BAPX_TOKEN is not set — cannot authenticate to bapx-api.')
    return c.json({
      error: 'Pipedream unavailable: BAPX_TOKEN is not configured. Restart the sandbox.',
    }, 503)
  }
    ...
    try {
      ...
      const res = await fetch(`${apiUrl}/pipedream/token`, {
        ...
        headers: {
          ...
          Authorization: `Bearer ${config.BAPX_TOKEN}`,
          ...getPipedreamHeaders(),
        },
...
pipedreamRouter.get('/credentials', async (c) => {
  try {
    const apiUrl = getApiV1()
    const res = await fetch(`${apiUrl}/pipedream/credentials`, {
      headers: { Authorization: `Bearer ${config.BAPX_TOKEN}` },
      signal: AbortSignal.timeout(15_000),
    })
    const data = await res.json()
    if (!res.ok) return c.json(data, res.status as any)
...
export async function pushPipedreamCredsToApi(): Promise<void> {
  ...
  if (!config.BAPX_TOKEN || !config.BAPX_API_URL) {
    console.log('[Pipedream] No BAPX_TOKEN/API_URL — skipping cred push')
    return
  }
...
async function upsertPipedreamConnector(app: string, appName?: string): Promise<string> {
  ...
  const root = process.env.BAPX_WORKSPACE || '/workspace'
...
export async function syncExistingConnectorsFromApi(): Promise<void> {
  if (!config.BAPX_TOKEN || !config.BAPX_API_URL) {
    console.log('[Pipedream] Connector boot-sync: no BAPX_TOKEN/API_URL — skipping')
    return
  }
  try {
    const apiUrl = getApiV1()
