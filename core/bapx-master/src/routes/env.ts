...
// POST /env/rotate-token — rotate BAPX_TOKEN.
// Encryption is decoupled from BAPX_TOKEN, so this just updates the token
// value and restarts services. No re-encryption needed.
envRouter.post('/rotate-token',
  describeRoute({
    tags: ['Secrets'],
    summary: 'Rotate BAPX_TOKEN',
    description: 'Rotates the BAPX_TOKEN used for sandbox↔API authentication. Encryption is decoupled — secrets are NOT re-encrypted. Always restarts services.',
    responses: {
  ...
  async (c) => {
    try {
      ...
      await writeS6Env('BAPX_TOKEN', newToken)
      updateBootstrapKey('BAPX_TOKEN', newToken)

      // Restart OpenCode to pick up the new token
      await restartServices(['opencode'])

      console.log(`[ENV API] BAPX_TOKEN rotated. ${result.rotated} secret(s) unaffected (encryption decoupled).`)
