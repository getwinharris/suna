...
const BAPX_MARKETPLACE_NAMESPACE = 'bapx'
const BAPX_MARKETPLACE_REGISTRY_URL = 'https://bapx-registry-6om.pages.dev'

function getWorkspacePaths() {
  const workspaceRoot = process.env.BAPX_WORKSPACE || '/workspace'
...
async function ensureBapxRegistry(): Promise<void> {
  ...
  const hasBapxRegistry =
    configText.includes('"bapx"') && configText.includes(BAPX_MARKETPLACE_REGISTRY_URL)

  if (!hasBapxRegistry) {
    await runOcx([
      'registry',
      'add',
      BAPX_MARKETPLACE_REGISTRY_URL,
      '--name',
      BAPX_MARKETPLACE_NAMESPACE,
    ])
  }
}
...
async function listInstalledMarketplaceComponents(): Promise<string[]> {
  ...
  return components
    .filter((component) => component.name.startsWith(`${BAPX_MARKETPLACE_NAMESPACE}/`))
...
marketplaceRouter.get('/status', async (c) => {
  try {
    ...
    const registryConfigured =
      ocxConfigText.includes('"bapx"') && ocxConfigText.includes(BAPX_MARKETPLACE_REGISTRY_URL)
    ...
    return c.json({
      ...
      status: {
        ...
        ocxConfigPath,
        opencodeConfigPath,
        registryUrl: BAPX_MARKETPLACE_REGISTRY_URL,
        registryConfigured,
        installedComponents,
      },
    })
...
marketplaceRouter.post('/install', async (c) => {
  try {
    ...
    const namespace = body.namespace || BAPX_MARKETPLACE_NAMESPACE
