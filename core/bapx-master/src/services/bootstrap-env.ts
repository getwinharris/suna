 ...
 *
 * Core vars (BAPX_TOKEN, BAPX_YOLO_API_KEY, BAPX_API_URL, INTERNAL_SERVICE_KEY,
 ...
 * The SecretStore can't help because BAPX_TOKEN is its own encryption key.
 *
...
const WORKSPACE_ROOT = process.env.BAPX_WORKSPACE_ROOT || '/workspace'
const BOOTSTRAP_PATH = process.env.BOOTSTRAP_PATH || `${WORKSPACE_ROOT}/.secrets/.bootstrap-env.json`

const CORE_VARS = ['BAPX_TOKEN', 'BAPX_YOLO_API_KEY', 'BAPX_YOLO_URL', 'BAPX_API_URL', 'INTERNAL_SERVICE_KEY', 'TUNNEL_TOKEN'] as const
 ...
 * the API rotates or re-issues BAPX_TOKEN. The /env API and injectSandboxToken
 ...
 *
 * Older builds allowed BAPX_TOKEN, BAPX_YOLO_API_KEY, INTERNAL_SERVICE_KEY,
...
export function normalizeBootstrapAuthAliases(): number {
  const canonical = process.env.BAPX_TOKEN || ''
  if (!canonical) return 0

  let updated = 0
  for (const key of ['BAPX_YOLO_API_KEY', 'INTERNAL_SERVICE_KEY', 'TUNNEL_TOKEN'] as const) {
  ...
  if (updated > 0) {
    console.log(`[Bootstrap] Normalized ${updated} auth alias(es) to BAPX_TOKEN`)
  }
  return updated
}
