...
const GATEWAY_TAGS: Array<{ name: string; description: string }> = [
  ...
  { name: 'GW: Secrets',      description: 'Environment variable / secret management (encrypted at rest via BAPX_TOKEN)' },
  ...
  const merged: OpenAPISpec = {
    ...
    info: {
      ...
      description:
        ...
        'Each sandbox gets an auto-generated token (`bapx_sb_` prefix, 42 chars) injected as the `BAPX_TOKEN` environment variable. ' +
    ...
    servers: [{ url: 'https://YOUR_BAPX_API/v1/p/{sandboxId}/8000', description: 'Replace with your Bapx API base URL' }],
    tags: [],
    paths: {},
    components: {
      schemas: {},
