...
export function buildOwnerPrompt(project: ProjectRow, task: TaskRow): string {
  return [
    ...
    `- id: ${project.id}`,
    `- path: ${project.path}`,
    `- context: ${project.path}/.bapx/CONTEXT.md`,
    '',
    'Task:',
    `- id: ${task.id}`,
    `- title: ${task.title}`,
    ...
    `- SECOND: read ${project.path}/.bapx/CONTEXT.md before making changes. Treat it as the shared project memory spine.`,
    ...
    '- Durable project docs (.bapx/CONTEXT.md) are maintained automatically by the hidden project-maintainer after each task event; you do not need to update them yourself.',
    '',
    'Completion contract:',
    '- The autowork loop stops when — and only when — you emit a <bapx_autowork_complete> tag with two children:',
  ...
  if (task.owner_session_id) {
    try {
      const resumePrompt = [
        ...
        'When complete, emit <bapx_autowork_complete> with <verification> and <requirements_check> children.',
      ].join('\n')
