...
interface SecretsData {
  ...
  /** Schema version. v1 = BAPX_TOKEN-derived key, v2 = dedicated encryption key. */
  version: number
}
 ...
 *
 * Encryption is decoupled from BAPX_TOKEN. A dedicated random encryption key
 * is generated on first use and stored at `.encryption-key` alongside the
 * secrets file. This means:
 *
 *   - Changing BAPX_TOKEN (API restart, rotation, pool claim) does NOT
 ...
 *   - Migration from v1 (BAPX_TOKEN-derived) happens automatically on
...
export class SecretStore {
  ...
  constructor() {
    const persistentRoot = process.env.BAPX_PERSISTENT_ROOT || '/persistent'
    ...
    // Dedicated encryption key — lives alongside secrets, independent of BAPX_TOKEN
  ...
  /**
   * Derive a key using the LEGACY method (BAPX_TOKEN + salt).
   * Only used during v1 → v2 migration.
   */
  private getLegacyKey(salt: Buffer): Buffer {
    return scryptSync(process.env.BAPX_TOKEN || 'default-key', salt, 32)
  }
  ...
  // ─── Migration from v1 (BAPX_TOKEN-derived encryption) ───────────────

  /**
   * Migrate secrets from v1 (encrypted with BAPX_TOKEN-derived key) to v2
  ...
  private async migrateFromV1(data: SecretsData): Promise<void> {
    const secretKeys = Object.keys(data.secrets)
    console.log(`[SecretStore] Migrating ${secretKeys.length} secret(s) from v1 (BAPX_TOKEN-derived) to v2 (dedicated key)...`)

    try {
      // Create backup BEFORE touching anything
      await this.createBackup('pre-v2-migration')

      // Try to decrypt with the legacy key (BAPX_TOKEN + salt)
      const salt = await this.getSalt()
      ...
      if (Object.keys(decrypted).length === 0) {
        console.warn('[SecretStore] Migration: could not decrypt any secrets with current BAPX_TOKEN. Secrets preserved as-is on disk.')
        return
      }
   ...
   * Rotate BAPX_TOKEN.
   *
   * Since encryption is now decoupled from BAPX_TOKEN, this method just
  ...
  async rotateToken(newToken: string): Promise<{ rotated: number }> {
    return this.withLock(async () => {
      ...
      process.env.BAPX_TOKEN = newToken

      console.log(`[SecretStore] BAPX_TOKEN rotated. ${count} secret(s) unaffected (encryption is decoupled).`)
