...
export const TEAM_SECTION_START = '<!-- BAPX:TEAM:START -->'
export const TEAM_SECTION_END = '<!-- BAPX:TEAM:END -->'
export const MILESTONES_SECTION_START = '<!-- BAPX:MILESTONES:START -->'
export const MILESTONES_SECTION_END = '<!-- BAPX:MILESTONES:END -->'
 ...
 *   2. `BAPX_DEFAULT_AGENT_MODEL` env
...
export function resolveDefaultModel(opts?: { override?: string | null }): string {
  ...
  const envOverride = process.env.BAPX_DEFAULT_AGENT_MODEL?.trim()
  if (envOverride) return envOverride
  if (process.env.ANTHROPIC_API_KEY) {
    return 'anthropic/claude-sonnet-4-6'
  }
  if (process.env.BAPX_YOLO_API_KEY && process.env.BAPX_YOLO_URL) {
    return 'bapx-yolo/think'
  }
  if (process.env.BAPX_TOKEN && process.env.BAPX_API_URL) {
...
async function pokeTriggerManagerReload(): Promise<void> {
  const port = process.env.BAPX_TRIGGER_WEBHOOK_PORT || '8099'
  try {
