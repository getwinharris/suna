...
const scriptPath = "/Users/markokraemer/Projects/heyagi/computer/core/bapx-master/opencode/skills/BAPX-system/connectors/integration.ts"
...
describe("integration.ts end-to-end through bapx-master", () => {
  ...
  beforeAll(async () => {
    ...
    master = await startBapxMaster(masterPort, fx, {
      ...
      BAPX_TOKEN: "test-bapx-token",
      BAPX_API_URL: `http://127.0.0.1:${apiPort}`,
  ...
  async function run(cmd: string, json?: object) {
    const proc = Bun.spawn(["bun", "run", scriptPath, cmd, ...(json ? [JSON.stringify(json)] : [])], {
      env: {
        ...process.env,
        BAPX_MASTER_URL: `http://127.0.0.1:${masterPort}`,
        INTERNAL_SERVICE_KEY: key,
      },
      stdout: "pipe",
      stderr: "pipe",
