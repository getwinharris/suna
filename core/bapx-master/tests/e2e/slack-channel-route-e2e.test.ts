...
describe('Slack channel webhook route', () => {
  const originalWorkspace = process.env.BAPX_WORKSPACE
  ...
  beforeEach(async () => {
    ...
    process.env.BAPX_WORKSPACE = workspace
    process.env.INTERNAL_SERVICE_KEY = 'bapx_sb_test'
    process.env.BAPX_TOKEN = 'bapx_sb_test'
    fetchCalls.length = 0
  ...
  afterEach(async () => {
    ...
    process.env.BAPX_WORKSPACE = originalWorkspace
