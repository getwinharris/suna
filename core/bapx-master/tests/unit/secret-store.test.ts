...
describe('SecretStore', () => {
  ...
  beforeEach(() => {
    ...
    savedEnv.BAPX_TOKEN = process.env.BAPX_TOKEN
    ...
    process.env.BAPX_TOKEN = 'test-token-for-encryption'
  })
  ...
  // ─── BAPX_TOKEN independence ──────────────────────────────────────────

  describe('encryption decoupled from BAPX_TOKEN', () => {
    it('secrets survive BAPX_TOKEN change', async () => {
      const store1 = new SecretStore()
      await store1.set('API_KEY', 'sk-test-123')
      await store1.set('OTHER_KEY', 'other-value')

      // Simulate BAPX_TOKEN change (API restart, rotation, etc.)
      process.env.BAPX_TOKEN = 'completely-different-token'

      // New store instance with new BAPX_TOKEN — secrets should still work
      const store2 = new SecretStore()
      expect(await store2.get('API_KEY')).toBe('sk-test-123')
      expect(await store2.get('OTHER_KEY')).toBe('other-value')
    })

    it('secrets survive BAPX_TOKEN being unset', async () => {
      const store1 = new SecretStore()
      await store1.set('IMPORTANT', 'must-survive')

      // Simulate BAPX_TOKEN being completely absent
      delete process.env.BAPX_TOKEN
    ...
    it('rotateToken does NOT break existing secrets', async () => {
      ...
      expect(process.env.BAPX_TOKEN).toBe('brand-new-token')
    })
  })
  ...
  describe('v1 → v2 migration', () => {
    /**
     * Helper: create a v1-style secrets file encrypted with BAPX_TOKEN-derived key.
    ...
    function createV1Secrets(token: string, secrets: Record<string, string>) {
      ...
      // Derive key the old way: scrypt(BAPX_TOKEN, salt, 32)
    ...
    it('automatically migrates v1 secrets to v2 on first access', async () => {
      // Create v1 secrets (encrypted with BAPX_TOKEN)
    ...
    it('migrated secrets survive BAPX_TOKEN change', async () => {
      createV1Secrets('test-token-for-encryption', {
      ...
      // Change token
      process.env.BAPX_TOKEN = 'new-completely-different-token'
