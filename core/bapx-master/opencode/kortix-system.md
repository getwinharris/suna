...
- The `<!-- BAPX:TASK-SUMMARY:START/END -->` block is machine-managed by `project_context_sync` — never hand-edit between those markers.
</authoring>
...
- Never an append-only dump. Summarize. Compress. Prune.
- The `<!-- BAPX:TASK-SUMMARY:START/END -->` block is machine-managed by `project_context_sync`. Don't hand-edit between those markers.
...
- `webhookHost: "0.0.0.0"`
- `webhookPort: BAPX_TRIGGER_WEBHOOK_PORT || 8099`
...
- Core vars (BAPX_TOKEN) are persisted to bootstrap for container restart survival
</environment>
