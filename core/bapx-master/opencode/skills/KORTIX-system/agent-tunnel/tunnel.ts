...
function getApiBase(): string {
  const raw = getEnv("BAPX_API_URL") || FALLBACK_API_URL;
  const url = raw.startsWith("http") ? raw : FALLBACK_API_URL;
  // Strip /v1/router suffix — BAPX_API_URL sometimes includes the router path
...
const client = new TunnelClient({
  ...
  token: getEnv("BAPX_TOKEN") || "",
  tunnelId: getEnv("BAPX_TUNNEL_ID"),
});
