...
async function enrichImages(images: EnrichedImage[]): Promise<EnrichedImage[]> {
  ...
  // When routed through the Bapx proxy (REPLICATE_API_URL is set), use BAPX_TOKEN
...
export default tool({
  ...
  async execute(args, _context) {
    ...
    // When routed through the Bapx proxy (SERPER_API_URL is set), use BAPX_TOKEN
