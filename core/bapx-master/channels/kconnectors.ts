...
function resolveDbPath(): string {
  const root = process.env.BAPX_WORKSPACE?.trim()
...
function resolveConnectorRoot(): string | null {
  // Check BAPX_WORKSPACE first — that's where runtime data lives in the sandbox
  const workspace = process.env.BAPX_WORKSPACE?.trim()
  if (workspace) {
