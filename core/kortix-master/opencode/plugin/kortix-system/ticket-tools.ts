...
export function ticketTools(db: Database, mgr: ProjectManager, client: any) {
  ...
  const scheduleMilestonesContextSync = (projectId: string): void => {
    ...
    syncMilestonesSection(db, proj).catch((err) => {
      console.warn('[bapx-tools] milestones CONTEXT.md sync failed:', err instanceof Error ? err.message : err)
    })
  }
  ...
  return {
    ...
    team_create_agent: tool({
      description: [
        ...
        'YAML frontmatter (name, description, mode, model + bapx metadata) is',
      ...
      args: {
        ...
        default_model: tool.schema.string().optional().describe('Model id in "providerID/modelID" form (e.g. "bapx-yolo/think", "bapx/minimax-m27", "anthropic/claude-sonnet-4-6"). Omit to inherit the project\'s seeded default (matches whatever provider the sandbox has credentials for).'),
      },
      async execute(args, ctx): Promise<string> {
        const pid = getProjectIdForCtx(mgr, ctx)
        if (!pid) return 'Error: no project selected.'
    ...
    team_update_agent: tool({
      ...
      async execute(args, ctx): Promise<string> {
        ...
        // If the agent was stored at the legacy `.bapx/agents/` path, clean
