...
CREATE TABLE IF NOT EXISTS bapx.pool_resources (
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_pool_resources_unique
  ON bapx.pool_resources (provider, server_type, location);

-- Pool inventory: actual pre-provisioned machines waiting to be claimed
CREATE TABLE IF NOT EXISTS bapx.pool_sandboxes (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  resource_id UUID REFERENCES bapx.pool_resources(id) ON DELETE SET NULL,
  provider bapx.sandbox_provider NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_pool_sandboxes_claim
  ON bapx.pool_sandboxes (status, created_at) WHERE status = 'ready';
