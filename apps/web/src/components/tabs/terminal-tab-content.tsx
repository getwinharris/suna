...
const SSHTerminal = dynamic(
  () => import('@/components/thread/bapx-computer/components/SSHTerminal').then(mod => ({ default: mod.SSHTerminal })),
  { ssr: false }
);
...
export function TerminalTabContent({ ptyId, tabId, hidden = false }: TerminalTabContentProps) {
  const currentSandboxId = useBapxComputerStore((s) => s.currentSandboxId);
