  ...
  const handleCta = useCallback(async () => {
    try {
      ...
      if (response.status === 'subscription_created' || response.status === 'no_change') {
        toast.success(response.message || 'Your Bapx is on its way');
        onOpenChange(false);
      ...
      >
        ...
        <div className="flex-1 min-h-0 overflow-y-auto">
          ...
          <div className="flex flex-col items-center pt-7 pb-5 px-6 border-b border-border bg-neutral-50/50 dark:bg-neutral-950/50">
            <Image src="/bapx-computer.png" alt="Bapx Computer" width={140} height={140} className="object-contain mb-4" priority />
            <h2 className="text-xl font-semibold tracking-tight text-foreground text-center">{title || 'Your Bapx'}</h2>
        ...
        <div className="shrink-0 border-t border-border px-5 py-4 flex items-center justify-between gap-4">
          ...
          <Button className="h-11 px-7 text-sm font-semibold" disabled={isLoading || !selected} onClick={handleCta}>
            {isLoading ? <Loader2 className="size-4 animate-spin" /> : <>Get Your Bapx<ArrowRight className="size-3.5 ml-1.5" /></>}
          </Button>
        </div>
      </div>
    </div>
