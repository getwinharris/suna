...
import { useBapxProject } from '@/hooks/bapx/use-bapx-projects';
import { openTabAndNavigate } from '@/stores/tab-store';
import { StatusPill } from '@/components/bapx/task-pills';
import {
  shortTaskId,
  relativeTime,
  fullDate,
} from '@/lib/bapx/task-meta';
import type { BapxTaskStatus } from '@/hooks/bapx/use-bapx-tasks';
...
export function TaskDetailView({
  ...
  const { data: task, isLoading } = useBapxTask(taskId || '', {
    enabled: !!taskId && pollingEnabled,
    pollingEnabled: !!taskId && pollingEnabled,
  });
  const { data: events } = useBapxTaskEvents(taskId || '', {
    enabled: !!taskId && pollingEnabled,
    pollingEnabled: !!taskId && pollingEnabled,
  });
  const { data: liveStatus } = useBapxTaskStatus(taskId || '', {
    enabled: !!taskId && pollingEnabled,
    pollingEnabled: !!taskId && pollingEnabled,
  });
  const { data: project } = useBapxProject(task?.project_id || '');
  const updateTask = useUpdateBapxTask();
  const startTask = useStartBapxTask();
  const approveTask = useApproveBapxTask();
  const deleteTaskMutation = useDeleteBapxTask();
...
const EVENT_META: Record<BapxTaskEvent['type'], EventMeta> = {
  progress: {
    icon: Activity,
    color: 'text-blue-500',
...
function Timeline({ events }: { events: BapxTaskEvent[] }) {
  return (
