...
/**
 * Project Channels tab — filtered view of /bapx/channels scoped to
...
async function projectChannelsFetch(serverUrl: string, path: string, opts?: RequestInit): Promise<any> {
  try {
    const res = await authenticatedFetch(`${serverUrl}/bapx/channels${path}`, opts);
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-[11px] text-muted-foreground truncate">
          ...
          {ch.default_agent && ch.default_agent !== 'bapx' ? ` · ${ch.default_agent}` : ''}
        </p>
      </div>
