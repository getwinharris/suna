...
export const trailbaseMFAService = {
  ...
  async enrollPhoneNumber(data: PhoneVerificationEnroll): Promise<EnrollFactorResponse> {
    ...
    try {
      const response = await trailbase.auth.mfa.enroll({
        factorType: 'phone',
  ...
  async createChallenge(data: PhoneVerificationChallenge): Promise<ChallengeResponse> {
    ...
    try {
      const response = await trailbase.auth.mfa.challenge({
  ...
  async verifyChallenge(data: PhoneVerificationVerify): Promise<PhoneVerificationResponse> {
    ...
    try {
      const response = await trailbase.auth.mfa.verify({
        factorId: data.factor_id,
  ...
  async challengeAndVerify(data: PhoneVerificationChallengeAndVerify): Promise<PhoneVerificationResponse> {
    ...
    try {
      const response = await trailbase.auth.mfa.challengeAndVerify({
        factorId: data.factor_id,
        code: data.code,
      });
  ...
  async listFactors(): Promise<ListFactorsResponse> {
    ...
    try {
      const { data: { user }, error } = await trailbase.auth.getUser();
  ...
  async unenrollFactor(factorId: string): Promise<PhoneVerificationResponse> {
    ...
    try {
      const response = await trailbase.auth.mfa.unenroll({
        factorId: factorId,
      });
  ...
  async getAAL(): Promise<AALResponse> {
    ...
    // If no active Trailbase session (e.g. pre-login), return safe defaults.
    ...
    try {
      const aalResponse = await trailbase.auth.mfa.getAuthenticatorAssuranceLevel();
      
      if (aalResponse.error) {
        throw new Error(aalResponse.error.message);
      }

      const { data: { user }, error: userError } = await trailbase.auth.getUser();
