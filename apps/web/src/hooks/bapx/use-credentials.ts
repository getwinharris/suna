...
export const credentialKeys = {
  list: (pid?: string) => ['bapx', 'credentials', pid ?? ''] as const,
  events: (pid: string, name: string) => ['bapx', 'credentials', pid, name, 'events'] as const,
};
...
export function useCredentials(projectId?: string) {
  ...
  return useQuery<CredentialItem[]>({
    ...
    queryFn: () =>
      kfetch<CredentialItem[]>(serverUrl, `/bapx/projects/${encodeURIComponent(projectId!)}/credentials`),
    enabled: !!projectId,
...
export function useCredentialEvents(projectId?: string, name?: string) {
  ...
  return useQuery<CredentialEvent[]>({
    ...
    queryFn: () =>
      kfetch<CredentialEvent[]>(serverUrl, `/bapx/projects/${encodeURIComponent(projectId!)}/credentials/${encodeURIComponent(name!)}/events`),
  ...
  return useMutation<CredentialWithValue, Error, { projectId: string; name: string }>({
    mutationFn: ({ projectId, name }) =>
      kfetch<CredentialWithValue>(
        serverUrl,
        `/bapx/projects/${encodeURIComponent(projectId)}/credentials/${encodeURIComponent(name)}`,
      ),
    onSuccess: (_res, vars) => {
