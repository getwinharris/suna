 ...
 *
 * Server shape comes from /bapx/projects/:projectId/milestones (see
 * core/bapx-master/src/routes/milestones.ts). The GET list returns
...
import type { Ticket } from './use-bapx-tickets';
...
async function kfetch<T>(serverUrl: string, apiPath: string, init?: RequestInit): Promise<T> {
  ...
  if (!res.ok) {
    ...
    throw new Error(`Bapx API ${res.status}: ${text.slice(0, 300)}`);
  }
  return res.json();
}

export const milestoneKeys = {
  list: (pid?: string, status: 'open' | 'closed' | 'all' = 'all') => ['bapx', 'milestones', pid ?? '', status] as const,
  detail: (pid: string, ref: string) => ['bapx', 'milestone', pid, ref] as const,
  events: (pid: string, ref: string) => ['bapx', 'milestone', pid, ref, 'events'] as const,
};
...
export function useMilestones(projectId?: string, statusFilter: 'open' | 'closed' | 'all' = 'all') {
  ...
  return useQuery<Milestone[]>({
    ...
    queryFn: () =>
      kfetch<Milestone[]>(serverUrl, `/bapx/projects/${encodeURIComponent(projectId!)}/milestones?status=${statusFilter}`),
    enabled: !!projectId,
...
export function useMilestone(projectId?: string, ref?: string) {
  ...
  return useQuery<MilestoneDetail>({
    ...
    queryFn: () =>
      kfetch<MilestoneDetail>(serverUrl, `/bapx/projects/${encodeURIComponent(projectId!)}/milestones/${encodeURIComponent(ref!)}`),
...
export function useMilestoneEvents(projectId?: string, ref?: string) {
  ...
  return useQuery<MilestoneEvent[]>({
    ...
    queryFn: () =>
      kfetch<MilestoneEvent[]>(serverUrl, `/bapx/projects/${encodeURIComponent(projectId!)}/milestones/${encodeURIComponent(ref!)}/events`),
    enabled: !!projectId && !!ref,
    refetchInterval: 5000,
    placeholderData: keepPreviousData,
  });
...
export function useCreateMilestone() {
  ...
  return useMutation<Milestone, Error, CreateMilestoneInput>({
    mutationFn: ({ projectId, ...body }) =>
      kfetch<Milestone>(serverUrl, `/bapx/projects/${encodeURIComponent(projectId)}/milestones`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(body),
      }),
    onSuccess: (_res, vars) => {
      qc.invalidateQueries({ queryKey: ['bapx', 'milestones', vars.projectId] });
    },
  });
}
...
export function useUpdateMilestone() {
  ...
  return useMutation<Milestone, Error, UpdateMilestoneInput>({
    mutationFn: ({ projectId, ref, patch }) =>
      kfetch<Milestone>(serverUrl, `/bapx/projects/${encodeURIComponent(projectId)}/milestones/${encodeURIComponent(ref)}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
...
export function useCloseMilestone() {
  ...
  return useMutation<Milestone, Error, { projectId: string; ref: string; summary_md?: string; cancelled?: boolean }>({
    mutationFn: ({ projectId, ref, summary_md, cancelled }) =>
      kfetch<Milestone>(serverUrl, `/bapx/projects/${encodeURIComponent(projectId)}/milestones/${encodeURIComponent(ref)}/close`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
export function useReopenMilestone() {
  ...
  return useMutation<Milestone, Error, { projectId: string; ref: string }>({
    mutationFn: ({ projectId, ref }) =>
      kfetch<Milestone>(serverUrl, `/bapx/projects/${encodeURIComponent(projectId)}/milestones/${encodeURIComponent(ref)}/reopen`, {
        method: 'POST',
      }),
    onSuccess: (_res, vars) => {
      qc.invalidateQueries({ queryKey: ['bapx', 'milestones', vars.projectId] });
...
export function useDeleteMilestone() {
  ...
  return useMutation<{ ok: boolean }, Error, { projectId: string; ref: string }>({
    mutationFn: ({ projectId, ref }) =>
      kfetch<{ ok: boolean }>(serverUrl, `/bapx/projects/${encodeURIComponent(projectId)}/milestones/${encodeURIComponent(ref)}`, {
        method: 'DELETE',
      }),
    onSuccess: (_res, vars) => {
      qc.invalidateQueries({ queryKey: ['bapx', 'milestones', vars.projectId] });
...
/** Link or unlink a ticket's milestone. Goes through PATCH /bapx/tickets/:id. */
export function useSetTicketMilestone() {
  ...
  return useMutation<unknown, Error, { projectId: string; ticketId: string; milestoneId: string | null }>({
    mutationFn: ({ ticketId, milestoneId }) =>
      kfetch(serverUrl, `/bapx/tickets/${encodeURIComponent(ticketId)}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
    ...
    onSuccess: (_res, vars) => {
      qc.invalidateQueries({ queryKey: ['bapx', 'milestones', vars.projectId] });
      qc.invalidateQueries({ queryKey: ['bapx', 'tickets', vars.projectId] });
      qc.invalidateQueries({ queryKey: ['bapx', 'ticket', vars.ticketId] });
    },
  });
}
