...
/**
 * Bapx v2 Tickets — hooks for tickets, columns, fields, templates, and
 * project agents (the "team"). Hits /bapx/tickets and /bapx/projects/:id/*.
 */
...
import { getUserHandle } from '@/lib/bapx/user-handle';
...
export const ticketKeys = {
  tickets: (pid?: string) => ['bapx', 'tickets', pid ?? ''] as const,
  ticket: (id: string) => ['bapx', 'ticket', id] as const,
  events: (id: string) => ['bapx', 'ticket', id, 'events'] as const,
  columns: (pid: string) => ['bapx', 'columns', pid] as const,
  fields: (pid: string) => ['bapx', 'fields', pid] as const,
  templates: (pid: string) => ['bapx', 'templates', pid] as const,
  agents: (pid: string) => ['bapx', 'agents', pid] as const,
};
...
export function useTickets(projectId?: string, opts?: { enabled?: boolean; pollingEnabled?: boolean }) {
  ...
  return useQuery<Ticket[]>({
    ...
    queryFn: () =>
      kfetch<Ticket[]>(serverUrl, `/bapx/tickets${projectId ? `?project_id=${encodeURIComponent(projectId)}` : ''}`),
...
export function useTicket(id?: string, opts?: { enabled?: boolean; pollingEnabled?: boolean }) {
  ...
  return useQuery<Ticket>({
    ...
    queryFn: () => kfetch<Ticket>(serverUrl, `/bapx/tickets/${encodeURIComponent(id!)}`),
...
export function useTicketEvents(id?: string, opts?: { enabled?: boolean; pollingEnabled?: boolean }) {
  ...
  return useQuery<TicketEvent[]>({
    ...
    queryFn: () => kfetch<TicketEvent[]>(serverUrl, `/bapx/tickets/${encodeURIComponent(id!)}/events`),
    enabled: !!id && (opts?.enabled ?? true),
    refetchInterval: opts?.pollingEnabled === false ? false : 3000,
    refetchIntervalInBackground: false,
    placeholderData: keepPreviousData,
...
export function useCreateTicket() {
  ...
  return useMutation({
    mutationFn: (body: {
      ...
      kfetch<{ ticket: Ticket; triggered: Array<{ agent_id: string; agent_slug: string; reason: string }> }>(serverUrl, '/bapx/tickets', {
        method: 'POST',
...
export function useUpdateTicket() {
  ...
  return useMutation({
    mutationFn: ({ id, ...body }: { id: string; title?: string; body_md?: string; template_id?: string | null; custom_fields?: Record<string, unknown>; milestone_id?: string | null }) =>
      kfetch<Ticket>(serverUrl, `/bapx/tickets/${encodeURIComponent(id)}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
...
export function useUpdateTicketStatus() {
  ...
  return useMutation({
    mutationFn: ({ id, status }: { id: string; status: string }) =>
      kfetch<{ ticket: Ticket; triggered: Array<{ agent_id: string; agent_slug: string; reason: string }> }>(serverUrl, `/bapx/tickets/${encodeURIComponent(id)}/status`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
export function useAssignTicket() {
  ...
  return useMutation({
    mutationFn: ({ id, assignee_type, assignee_id }: { id: string; assignee_type: AssigneeType; assignee_id: string }) =>
      kfetch<{ added: boolean; ticket: Ticket }>(serverUrl, `/bapx/tickets/${encodeURIComponent(id)}/assign`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
export function useUnassignTicket() {
  ...
  return useMutation({
    mutationFn: ({ id, assignee_type, assignee_id }: { id: string; assignee_type: AssigneeType; assignee_id: string }) =>
      kfetch<{ removed: boolean; ticket: Ticket }>(serverUrl, `/bapx/tickets/${encodeURIComponent(id)}/unassign`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ assignee_type, assignee_id, actor_type: 'user', actor_id: handle }),
      }),
...
export function useCommentTicket() {
  ...
  return useMutation({
    mutationFn: ({ id, body }: { id: string; body: string }) =>
      kfetch<{ event: TicketEvent; mentions: string[]; triggered: Array<{ agent_slug: string }> }>(serverUrl, `/bapx/tickets/${encodeURIComponent(id)}/comments`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ body, actor_type: 'user', actor_id: handle }),
      }),
...
export function useDeleteTicket() {
  ...
  return useMutation({
    mutationFn: (id: string) =>
      kfetch<{ deleted: true }>(serverUrl, `/bapx/tickets/${encodeURIComponent(id)}`, { method: 'DELETE' }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['bapx', 'tickets'] }); },
  });
}
...
export function useColumns(projectId?: string) {
  ...
  return useQuery<TicketColumn[]>({
    ...
    queryFn: () => kfetch<TicketColumn[]>(serverUrl, `/bapx/projects/${encodeURIComponent(projectId!)}/columns`),
    enabled: !!projectId,
    staleTime: 30_000,
  });
}
...
export function useReplaceColumns() {
  ...
  return useMutation({
    mutationFn: ({ projectId, columns }: { projectId: string; columns: Array<{ key: string; label: string; default_assignee_type?: AssigneeType | null; default_assignee_id?: string | null; is_terminal?: boolean; is_off_flow?: boolean; icon?: string | null }> }) =>
      kfetch<TicketColumn[]>(serverUrl, `/bapx/projects/${encodeURIComponent(projectId)}/columns`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
...
export function useFields(projectId?: string) {
  ...
  return useQuery<ProjectField[]>({
    ...
    queryFn: () => kfetch<ProjectField[]>(serverUrl, `/bapx/projects/${encodeURIComponent(projectId!)}/fields`),
    enabled: !!projectId,
    staleTime: 30_000,
  });
}
...
export function useReplaceFields() {
  ...
  return useMutation({
    mutationFn: ({ projectId, fields }: { projectId: string; fields: Array<{ key: string; label: string; type: 'text' | 'number' | 'date' | 'select'; options?: string[] | null }> }) =>
      kfetch<ProjectField[]>(serverUrl, `/bapx/projects/${encodeURIComponent(projectId)}/fields`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
...
export function useTemplates(projectId?: string) {
  ...
  return useQuery<TicketTemplate[]>({
    ...
    queryFn: () => kfetch<TicketTemplate[]>(serverUrl, `/bapx/projects/${encodeURIComponent(projectId!)}/templates`),
    enabled: !!projectId,
    staleTime: 30_000,
  });
}
...
export function useReplaceTemplates() {
  ...
  return useMutation({
    mutationFn: ({ projectId, templates }: { projectId: string; templates: Array<{ name: string; body_md: string }> }) =>
      kfetch<TicketTemplate[]>(serverUrl, `/bapx/projects/${encodeURIComponent(projectId)}/templates`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
...
export function useEnsurePmSession() {
  ...
  return useMutation({
    mutationFn: ({ projectId }: { projectId: string }) =>
      kfetch<{ session_id: string; reused: boolean }>(serverUrl, `/bapx/projects/${encodeURIComponent(projectId)}/pm-session`, {
        method: 'POST',
      }),
  });
}
...
export function useProjectAgents(projectId?: string) {
  ...
  return useQuery<ProjectAgent[]>({
    ...
    queryFn: () => kfetch<ProjectAgent[]>(serverUrl, `/bapx/projects/${encodeURIComponent(projectId!)}/agents`),
    enabled: !!projectId,
    staleTime: 30_000,
  });
}
...
export function useCreateProjectAgent() {
  ...
  return useMutation({
    mutationFn: ({
      ...
      kfetch<ProjectAgent>(serverUrl, `/bapx/projects/${encodeURIComponent(projectId)}/agents`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
export function useUpdateProjectAgent() {
  ...
  return useMutation({
    mutationFn: ({ projectId, slug, ...body }: {
      ...
      kfetch<ProjectAgent>(serverUrl, `/bapx/projects/${encodeURIComponent(projectId)}/agents/${encodeURIComponent(slug)}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
...
export function useDeleteProjectAgent() {
  ...
  return useMutation({
    mutationFn: ({ projectId, slug }: { projectId: string; slug: string }) =>
      kfetch<{ deleted: true }>(serverUrl, `/bapx/projects/${encodeURIComponent(projectId)}/agents/${encodeURIComponent(slug)}`, {
        method: 'DELETE',
      }),
    onSuccess: (_d, vars) => { qc.invalidateQueries({ queryKey: ticketKeys.agents(vars.projectId) }); },
  });
...
export function useAgentPersona(projectId?: string, slug?: string, opts?: { enabled?: boolean }) {
  ...
  return useQuery<{ agent: ProjectAgent; body_md: string }>({
    queryKey: ['bapx', 'agent-persona', projectId ?? '', slug ?? ''],
    queryFn: () =>
      kfetch<{ agent: ProjectAgent; body_md: string }>(serverUrl, `/bapx/projects/${encodeURIComponent(projectId!)}/agents/${encodeURIComponent(slug!)}/persona`),
...
export function useProjectActivity(projectId?: string, opts?: { enabled?: boolean; pollingEnabled?: boolean }) {
  const serverUrl = useServerStore((s) => s.getActiveServerUrl());
  return useQuery<TicketEvent[]>({
    queryKey: ['bapx', 'project-activity', projectId ?? ''],
    queryFn: () =>
      kfetch<TicketEvent[]>(serverUrl, `/bapx/projects/${encodeURIComponent(projectId!)}/activity?limit=200`),
    enabled: !!projectId && (opts?.enabled ?? true),
...
const LAST_SEEN_KEY = (projectId: string, handle: string) => `bapx:activity-last-seen:${projectId}:${handle}`;
export const LAST_SEEN_EVENT = 'bapx:last-seen-changed';
