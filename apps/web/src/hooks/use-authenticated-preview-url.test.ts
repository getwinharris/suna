...
describe('buildPreviewAuthEndpoint', () => {
  test('builds auth URLs from canonical preview paths', () => {
    expect(buildPreviewAuthEndpoint('http://localhost:8008/v1/p/bapx-sandbox/4502/index.html'))
  ...
  test('uses the active server URL for already-proxied preview URLs', () => {
    expect(buildPreviewAuthEndpoint(
      'http://localhost:8000/proxy/4502/v1/p/bapx-sandbox/4502/index.html',
      'http://localhost:8008/v1/p/bapx-sandbox/8000',
    )).toBe('http://localhost:8008/v1/p/auth');
  });

  test('never nests auth requests under a proxied app path', () => {
    expect(buildPreviewAuthEndpoint('http://localhost:8000/proxy/4502/v1/p/bapx-sandbox/4502/index.html'))
