...
export function useAuth() {
      ...
      try {
        ...
        const { data, error: signUpError } = await trailbase.auth.signUp({
          ...
          options: {
            ...
            emailRedirectTo: 'bapx://auth/callback',
          },
        });
   ...
   * Uses bapx:// deep link - works when app is installed
   */
  const signInWithMagicLink = useCallback(async ({ email, acceptedTerms }: { email: string; acceptedTerms?: boolean }) => {
    try {
      ...
      const emailRedirectTo = `bapx://auth/callback${params.toString() ? `?${params.toString()}` : ''}`;
      ...
      if (magicLinkError) {
        log.error('❌ Supabase rejected redirect URL:', {
          ...
          hint: 'Make sure bapx://auth/callback is in Supabase Dashboard → Auth → Redirect URLs',
        });
      }
