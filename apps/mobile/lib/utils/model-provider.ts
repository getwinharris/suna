...
import BapxSymbolIcon from '@/assets/brand/bapx-symbol.svg';
...
export function isBapxMode(modelId: string): boolean {
  ...
  if (modelId === 'bapx/basic' || modelId === 'bapx/power' ||
      modelId === 'bapx-basic' || modelId === 'bapx-power') {
    return true;
  }
...
export function getModelProvider(modelId: string): ModelProvider {
  ...
  if (parts.length > 1) {
    ...
    if (['openai', 'anthropic', 'google', 'xai', 'moonshotai', 'bedrock', 'openrouter', 'bapx'].includes(provider)) {
...
export function getModelProviderIcon(modelId: string): React.FC<SvgProps> {
  ...
  const iconMap: Record<ModelProvider, React.FC<SvgProps>> = {
    bapx: BapxSymbolIcon, // Bapx modes use the Bapx symbol
