...
try {
  const [stats] = await sql<{
    ...
    select
      ...
      (
        select count(*)::text
        from legacy l
        left join bapx.billing_customers k on k.id = l.id
        where k.id is null
      ) as missing_bapx_rows,
      (
        select count(*)::text
        from legacy l
        join bapx.billing_customers k on k.account_id = l.account_id
        where k.id <> l.id
  ...
  if (!apply) {
    console.log('Dry run only. Re-run with --apply to sync basejump Stripe customers into bapx.billing_customers.');
    process.exit(0);
  }
  ...
  const [result] = await sql<{
    ...
    with legacy as (
      ...
      insert into bapx.billing_customers (account_id, id, email, active, provider)
      ...
      update bapx.billing_customers k
      set active = false
      from legacy l
