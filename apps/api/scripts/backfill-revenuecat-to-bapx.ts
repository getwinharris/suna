...
try {
  const [stats] = await sql<{
    ...
    missing_bapx_rows: string;
    placeholder_bapx_rows: string;
  }[]>`
    with legacy as (
      select *
    ...
    select
      ...
      (
        select count(*)::text
        from legacy l
        left join bapx.credit_accounts k on k.account_id = l.account_id
        where k.account_id is null
      ) as missing_bapx_rows,
      (
        select count(*)::text
        from legacy l
        join bapx.credit_accounts k on k.account_id = l.account_id
        where coalesce(k.tier, 'free') in ('free', 'none')
      ) as placeholder_bapx_rows
  `;

  console.log(JSON.stringify({ mode: apply ? 'apply' : 'dry-run', ...stats }, null, 2));

  if (!apply) {
    console.log('Dry run only. Re-run with --apply to sync RevenueCat legacy rows into bapx.credit_accounts.');
    process.exit(0);
  }
  ...
  const result = await sql<{
    ...
    with legacy as (
      ...
      set
        ...
        revenuecat_customer_id = coalesce(excluded.revenuecat_customer_id, bapx.credit_accounts.revenuecat_customer_id),
        revenuecat_subscription_id = coalesce(excluded.revenuecat_subscription_id, bapx.credit_accounts.revenuecat_subscription_id),
        ...
        balance = case
          when coalesce(bapx.credit_accounts.tier, 'free') in ('free', 'none')
            and coalesce(bapx.credit_accounts.balance, 0) = 0
          then excluded.balance
          else bapx.credit_accounts.balance
        end,
        expiring_credits = case
          when coalesce(bapx.credit_accounts.tier, 'free') in ('free', 'none')
            and coalesce(bapx.credit_accounts.expiring_credits, 0) = 0
          then excluded.expiring_credits
          else bapx.credit_accounts.expiring_credits
        end,
        non_expiring_credits = case
          when coalesce(bapx.credit_accounts.tier, 'free') in ('free', 'none')
            and coalesce(bapx.credit_accounts.non_expiring_credits, 0) = 0
          then excluded.non_expiring_credits
          else bapx.credit_accounts.non_expiring_credits
        end,
        daily_credits_balance = case
          when coalesce(bapx.credit_accounts.tier, 'free') in ('free', 'none')
            and coalesce(bapx.credit_accounts.daily_credits_balance, 0) = 0
          then excluded.daily_credits_balance
          else bapx.credit_accounts.daily_credits_balance
        end
      returning 1
    )
