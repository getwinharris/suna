...
import { integrationCredentials } from '@bapx/db';
import {
  getTestDb,
  getTestAccountIds,
  HAS_SAFE_TEST_DB,
...
async function ensureTestAccount(userId: string): Promise<string> {
  ...
  const [existing] = await db.execute(
    sql`SELECT am.account_id FROM bapx.account_members am WHERE am.user_id = ${userId} LIMIT 1`
  );
  ...
  const [acc] = await db.execute(
    sql`INSERT INTO bapx.accounts (name, personal_account) VALUES ('Test Account', true) RETURNING account_id`
  );
  const accountId = (acc as any).account_id;
  await db.execute(
    sql`INSERT INTO bapx.account_members (user_id, account_id, account_role) VALUES (${userId}, ${accountId}, 'owner')`
  );
  return accountId;
}
...
function createPipedreamTestApp(opts: { userId?: string; accountId?: string } = {}) {
  ...
  // Auth stub — simulates trailbaseAuth / apiKeyAuth
...
describe.skipIf(!HAS_DB)('Full flow: sandbox push → DB → frontend resolve (e2e)', () => {
  ...
  it('creds saved via PUT are retrievable and flagged as account source', async () => {
    // Step 1: sandbox pushes creds to API (simulates bapx-master boot push)
