/**
 * Bapx API proxy handler — /v1/bapx/*
 *
...
export async function bapxProxyHandler(c: Context): Promise<Response> {
  // /v1/bapx/projects/xxx → /bapx/projects/xxx
  const sandboxPath = c.req.path.replace(/^\/v1/, '').replace(/\/+$/, '') || '/bapx';
