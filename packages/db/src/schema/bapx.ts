...
export const bapxSchema = pgSchema('bapx');

export const sandboxStatusEnum = bapxSchema.enum('sandbox_status', [
  'provisioning',
  'active',
  'stopped',
  'archived',
...
export const sandboxProviderEnum = bapxSchema.enum('sandbox_provider', [
  'daytona',
  'local_docker',
  'justavps',
]);

export const deploymentStatusEnum = bapxSchema.enum('deployment_status', [
  'pending',
  'building',
  'deploying',
  'active',
...
export const deploymentSourceEnum = bapxSchema.enum('deployment_source', [
  'git',
  'code',
  'files',
  'tar',
...
export const apiKeyStatusEnum = bapxSchema.enum('api_key_status', [
  'active',
  'revoked',
  'expired',
]);

export const apiKeyTypeEnum = bapxSchema.enum('api_key_type', [
  'user',
  'sandbox',
]);

export const integrationStatusEnum = bapxSchema.enum('integration_status', [
  'active',
  'revoked',
  'expired',
  'error',
...
// ─── Accounts & Members ─────────────────────────────────────────────────────
// Replaces basejump.account_user. Fully bapx-native.

export const accountRoleEnum = bapxSchema.enum('account_role', [
  'owner',
  'admin',
  'member',
]);

export const accounts = bapxSchema.table(
  'accounts',
  {
...
export const accountMembers = bapxSchema.table(
  'account_members',
  {
...
export const sandboxes = bapxSchema.table(
  'sandboxes',
  {
...
export const scopeEffectEnum = bapxSchema.enum('scope_effect', [
  'grant',
  'revoke',
]);

export const sandboxMembers = bapxSchema.table(
  'sandbox_members',
  {
...
export const sandboxMemberScopes = bapxSchema.table(
...
export const sandboxInvites = bapxSchema.table(
  'sandbox_invites',
  {
...
export const poolResources = bapxSchema.table(
  'pool_resources',
  {
...
export const poolSandboxes = bapxSchema.table(
  'pool_sandboxes',
  {
    id: uuid('id').defaultRandom().primaryKey(),
...
export const deployments = bapxSchema.table(
  'deployments',
  {
...
export const bapxApiKeys = bapxSchema.table(
  'api_keys',
  {
  ...
  (table) => [
    uniqueIndex('idx_bapx_api_keys_public_key').on(table.publicKey),
    index('idx_bapx_api_keys_secret_hash').on(table.secretKeyHash),
    index('idx_bapx_api_keys_sandbox').on(table.sandboxId),
    index('idx_bapx_api_keys_account').on(table.accountId),
  ],
);
...
export const integrationCredentials = bapxSchema.table(
...
export const integrations = bapxSchema.table(
  'integrations',
  {
...
export const sandboxIntegrations = bapxSchema.table(
...
export const serverEntries = bapxSchema.table(
  'server_entries',
  {
...
export const oauthClients = bapxSchema.table(
  'oauth_clients',
  {
...
export const oauthAuthorizationCodes = bapxSchema.table(
...
export const oauthAccessTokens = bapxSchema.table(
  'oauth_access_tokens',
  {
    id: uuid('id').defaultRandom().primaryKey(),
...
export const oauthRefreshTokens = bapxSchema.table(
...
export const bapxApiKeysRelations = relations(bapxApiKeys, ({ one }) => ({
  sandbox: one(sandboxes, {
    fields: [bapxApiKeys.sandboxId],
    references: [sandboxes.sandboxId],
  }),
}));
...
export const billingCustomers = bapxSchema.table(
  'billing_customers',
  {
    accountId: uuid('account_id').notNull(),
  ...
  (table) => [
    index('idx_bapx_billing_customers_account_id').on(table.accountId),
  ],
);

export const creditAccounts = bapxSchema.table(
  'credit_accounts',
  {
  ...
  (table) => [
    index('bapx_credit_accounts_account_id_idx').on(table.accountId),
  ],
);

export const creditLedger = bapxSchema.table(
  'credit_ledger',
  {
  ...
  (table) => [
    unique('bapx_unique_stripe_event').on(table.stripeEventId),
    index('idx_bapx_credit_ledger_idempotency')
      .on(table.idempotencyKey)
      .where(sql`${table.idempotencyKey} IS NOT NULL`),
  ],
);

export const creditUsage = bapxSchema.table('credit_usage', {
  id: uuid().defaultRandom().primaryKey().notNull(),
  accountId: uuid('account_id').notNull(),
...
export const accountDeletionRequests = bapxSchema.table('account_deletion_requests', {
  id: uuid().defaultRandom().primaryKey().notNull(),
  accountId: uuid('account_id').notNull(),
  userId: uuid('user_id').notNull(),
...
export const creditPurchases = bapxSchema.table('credit_purchases', {
  id: uuid().defaultRandom().primaryKey().notNull(),
  accountId: uuid('account_id').notNull(),
  amountDollars: numeric('amount_dollars', { precision: 10, scale: 2 }).notNull(),
...
export const tunnelStatusEnum = bapxSchema.enum('tunnel_status', [
  'online',
  'offline',
  'connecting',
]);

export const tunnelCapabilityEnum = bapxSchema.enum('tunnel_capability', [
  'filesystem',
  'shell',
  'network',
  'apps',
...
export const tunnelPermissionStatusEnum = bapxSchema.enum('tunnel_permission_status', [
  'active',
  'revoked',
  'expired',
]);

export const tunnelPermissionRequestStatusEnum = bapxSchema.enum('tunnel_permission_request_status', [
  'pending',
  'approved',
  'denied',
  'expired',
...
export const tunnelConnections = bapxSchema.table(
  'tunnel_connections',
  {
...
export const tunnelPermissions = bapxSchema.table(
  'tunnel_permissions',
  {
...
export const tunnelPermissionRequests = bapxSchema.table(
...
export const tunnelAuditLogs = bapxSchema.table(
  'tunnel_audit_logs',
  {
...
export const tunnelDeviceAuthStatusEnum = bapxSchema.enum('tunnel_device_auth_status', [
  'pending',
  'approved',
  'denied',
  'expired',
...
export const tunnelDeviceAuthRequests = bapxSchema.table(
...
export const platformRoleEnum = bapxSchema.enum('platform_role', [
  'user',
  'admin',
  'super_admin',
]);

export const platformUserRoles = bapxSchema.table(
  'platform_user_roles',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    accountId: uuid('account_id').notNull(),
...
export const accessRequestStatusEnum = bapxSchema.enum('access_request_status', [
  'pending',
  'approved',
  'rejected',
]);

export const platformSettings = bapxSchema.table(
  'platform_settings',
  {
...
export const accessAllowlist = bapxSchema.table(
  'access_allowlist',
  {
    id: uuid('id').defaultRandom().primaryKey(),
...
export const accessRequests = bapxSchema.table(
  'access_requests',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    email: varchar('email', { length: 255 }).notNull(),
