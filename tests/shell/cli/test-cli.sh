#!/usr/bin/env bash
# ╔══════════════════════════════════════════════════════════════════════════════╗
# ║  Test Suite: Embedded CLI (generated by get-bapx.sh)                     ║
# ╚══════════════════════════════════════════════════════════════════════════════╝
...
# Extract the embedded CLI from get-bapx.sh (between CLIPATH markers)
CLI_CONTENT=$(sed -n "/cat > .*\/bapx.*<< 'CLIPATH'/,/^CLIPATH/p" "$SCRIPT" | sed '1d;$d')

if [ -z "$CLI_CONTENT" ]; then
  fail "could not extract embedded CLI from get-bapx.sh"
  echo ""; echo "  0/$TOTAL passed"; echo ""
  exit 1
fi

pass "embedded CLI extracted from get-bapx.sh"
